...
public class _2013211392_阿尔斯兰_7_StrategyUI extends JFrame{
	...
	public _2013211392_阿尔斯兰_7_StrategyUI() throws HeadlessException {
		...
		add_c.addActionListener(event -> {
			int tmp2 = 0;
			for(Checkbox strat : strats){
				if (strat.getState()){
					for(int j = 0;j < 5; j++){
						if (strat.getLabel().compareTo(booktype[j])==0){
							tmp2 = tmp2*10+(j+1);
						}
					}
				}
			}
			_2013211392_阿尔斯兰_7_MainUI.controller.addCompositeStrategy(comboTypesList_c.getSelectedIndex()+1, tmp2);
			JOptionPane.showMessageDialog(null, "添加成功", "成功", JOptionPane.INFORMATION_MESSAGE);
			this.dispose();
			new _2013211392_阿尔斯兰_7_StrategyUI().setVisible(true);
		});
